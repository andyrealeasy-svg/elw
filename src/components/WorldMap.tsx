...
const REGIONS: MapRegion[] = [
  {
    ...
    bgGradient: "from-red-950/40 via-[#111111]/50 to-[#0a0a0a]/80",
  ...
  },
  {
    ...
    bgGradient: "from-cyan-950/40 via-[#111111]/50 to-[#0a0a0a]/80",
  ...
  },
  {
    ...
    bgGradient: "from-indigo-950/40 via-[#111111]/50 to-[#0a0a0a]/80",
  ...
  },
  {
    ...
    bgGradient: "from-emerald-950/40 via-[#111111]/50 to-[#0a0a0a]/80",
  ...
  },
  {
    ...
    bgGradient: "from-purple-950/40 via-[#111111]/50 to-[#0a0a0a]/80",
...
export default function WorldMap({ profile, updateProfile, setRoute, onBack }: Props) {
  ...
  return (
    <div className="w-full max-w-6xl h-[95dvh] md:h-[85vh] bg-[#0a0a0a] rounded-3xl border border-white/5 shadow-2xl flex flex-col overflow-hidden relative text-white/90 font-sans">
      ...
      {/* Header */}
      <div className="p-4 sm:p-5 border-b border-white/5 flex items-center justify-between bg-[#111111]/40 relative z-20 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5 text-[9px] sm:text-[10px] font-mono text-white/50 uppercase tracking-widest leading-none">
        ...
        <button 
          onClick={onBack}
          className="px-4 py-2 sm:px-6 bg-[#1a1a1a] hover:bg-white/10 border border-white/10 text-white font-bold rounded-2xl transition-all active:scale-95 text-xs uppercase tracking-wider"
        >
          Назад
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative z-10">
        ...
        {/* Main Canvas Area */}
        <div className="flex-1 min-h-[40vh] md:min-h-0 relative bg-[#0a0a0a]/40 overflow-hidden border-b-2 md:border-b-0 md:border-r border-white/5 flex items-center justify-center">
                ...
                >
                  <div className="relative">
                    ...
                    <div className={cn(
                      ...
                      selected ? "border-amber-400 bg-[#111111] shadow-amber-500/20" :
                      unlocked 
                        ? "border-white/10 bg-[#111111] hover:border-indigo-400" 
                  ...
                  <div className={cn(
                    "mt-2.5 px-3 py-1 bg-[#111111]/90 backdrop-blur-md rounded-xl border text-center transition-colors shadow-xl",
                    selected ? "border-amber-500/50 text-white" : "border-white/5 text-white/50 group-hover:text-white"
                  )}>
          ...
          <div className="absolute bottom-4 left-4 bg-[#111111]/90 backdrop-blur-xl border border-white/5 p-3 rounded-2xl max-w-xs text-xs pointer-events-none space-y-1 shadow-2xl">
        ...
        <div className="w-full md:w-2/5 shrink-0 bg-[#111111]/40 p-5 overflow-y-auto max-h-[50vh] md:max-h-none flex flex-col relative z-20">
          <AnimatePresence mode="wait">
            {selectedRegion ? (
              ...
              >
                {/* Visual Banner */}
                <div className={cn("p-4 rounded-3xl border bg-gradient-to-br shadow-inner text-left relative overflow-hidden", selectedRegion.bgGradient, selectedRegion.elementColor)}>
                   <div className="relative z-10">
                     ...
                     <p className="text-sm text-white/70 font-medium leading-relaxed mt-1">{selectedRegion.description}</p>
                   </div>
                ...
                {isRegionUnlocked(selectedRegion) ? (
                  <div className="space-y-5">
                    {/* Dungeons Panel */}
                    <div>
                      <h4 className="text-[11px] font-black uppercase text-white/40 tracking-widest mb-2.5">Доступные испытания</h4>
                      <div className="grid grid-cols-1 gap-2.5">
                        {selectedRegion.dungeons.map(dung => (
                          <div 
                            key={dung.id}
                            className="p-3 bg-[#0a0a0a] rounded-2xl border border-white/5 hover:border-indigo-500/20 flex justify-between items-center transition-all group"
                          >
                            <div className="min-w-0 pr-4">
                              <span className="font-bold text-white/90 text-sm block truncate group-hover:text-indigo-400">{dung.name}</span>
                              <span className="text-[10px] text-white/40 font-mono mt-0.5 block truncate">{dung.desc}</span>
                            </div>
                            <button
                              onClick={() => setRoute('ARTIFACT_DUNGEON_SELECTOR')}
                              className="px-3.5 py-1.5 bg-indigo-900/30 border border-indigo-700 hover:bg-indigo-600 transition text-white rounded-xl font-mono font-bold text-[10px] uppercase shrink-0"
                            >
                              Перейти
                            </button>
                          </div>
                    ...
                    <div>
                      <h4 className="text-[11px] font-black uppercase text-white/40 tracking-widest mb-2.5">Сокровища региона</h4>
                      <div className="grid grid-cols-1 gap-2.5">
                        {selectedRegion.chests.map(chest => {
                          ...
                          return (
                            <div 
                              key={chest.id}
                              className={cn(
                                "p-3 bg-[#0a0a0a] rounded-2xl border transition-all flex justify-between items-center",
                                claimed ? "border-white/5 opacity-60" : "border-white/5"
                              )}
                            >
                              <div className="min-w-0 pr-4">
                                <span className={cn("font-bold text-sm block truncate", claimed ? "text-white/40 line-through" : "text-white/90")}>{chest.name}</span>
                                <div className="flex items-center gap-1.5 text-[9px] font-mono text-white/50 mt-0.5">
                                  <span>Награда:</span>
                              ...
                              <button
                                ...
                                className={cn(
                                  "px-3 py-1.5 rounded-xl font-mono font-bold text-[10px] uppercase shrink-0 transition-all",
                                  claimed 
                                    ? "bg-[#111111] text-white/40 border border-white/5" 
                                    : "bg-amber-600 hover:bg-amber-500 text-black hover:scale-105 active:scale-95"
                                )}
                              >
                    ...
                    <div className="p-4 bg-indigo-950/20 border border-indigo-500/20 rounded-3xl">
                       ...
                       <p className="text-xs text-white/50 leading-relaxed mt-1 font-mono">
                       ...
                       <div className="flex flex-wrap gap-2 items-center my-3">
                          <span className="text-[10px] font-bold text-white/40 font-mono uppercase tracking-wider">Группа врагов ({selectedRegion.anomaly.level} lvl):</span>
                          <div className="flex gap-1.5">
                             {selectedRegion.anomaly.enemyIcons.map((ic, index) => (
                               <span key={index} className="w-7 h-7 bg-[#111111] border border-white/5 rounded-xl flex items-center justify-center text-sm shadow-md" title={selectedRegion.anomaly.enemies[index]}>
                                 {ic}
                               </span>
                             ))}
                          </div>
                       ...
                       <div className="flex items-end justify-between pt-2 border-t border-indigo-500/10">
                          ...
                          <button
                            ...
                            className="px-5 py-2.5 bg-gradient-to-r from-red-600 to-indigo-600 hover:from-red-500 hover:to-indigo-500 text-white font-extrabold text-xs uppercase rounded-2xl tracking-wider shadow-lg shadow-indigo-500/20 hover:scale-105 active:scale-95 transition-all"
                          >
                             Вызвать Глитч
                          </button>
                       </div>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-[#0a0a0a]/60 rounded-3xl border border-white/5 border-dashed">
                    <Lock className="w-12 h-12 text-white/20 mb-3 animate-bounce" />
                    <h4 className="font-bold text-white/40 uppercase text-xs tracking-wider">Сектор заблокирован</h4>
                    <p className="text-xs text-white/40 font-mono mt-1 px-4 leading-relaxed">
      ...
      <AnimatePresence>
        {claimSuccessMsg && (
          ...
          >
            <div className="bg-[#111111] border-2 border-amber-500 p-6 rounded-3xl text-center shadow-[0_0_30px_rgba(245,158,11,0.3)] max-w-sm">
               ...
               <p className="text-sm text-white/70 font-mono mt-2 whitespace-pre-wrap leading-relaxed">{claimSuccessMsg}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
