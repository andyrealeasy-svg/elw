...
export const MetaGuide: React.FC<MetaGuideProps> = ({ onBack }) => {
  ...
  const getTierInfo = (tier: string) => {
    switch(tier) {
      ...
      case 'T2': return { label: 'Средние', desc: 'Хороши в своих нишах, требуют специфических условий.', color: 'bg-white/10 text-white/70' };
      case 'T3': return { label: 'Слабые / Нишевые', desc: 'Требуют огромных вложений или очень специфических отрядов.', color: 'bg-[#1a1a1a] text-white/40' };
      default: return { label: 'Неизвестно', desc: '', color: 'bg-[#1a1a1a] text-white' };
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <div className="relative z-10 flex items-center justify-between p-6 border-b border-white/5 bg-[#111111]/50 backdrop-blur-xl">
        ...
        <div className="flex bg-[#1a1a1a]/50 rounded-3xl p-1 gap-1">
          {(['TIER_LIST', 'TEAMS', 'REACTIONS'] as const).map(tab => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={cn(
                "px-4 sm:px-6 py-2 rounded-2xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all",
                activeTab === tab ? "bg-indigo-500 text-white shadow-lg" : "text-white/50 hover:text-white"
              )}
            >
            ...
            {activeTab === 'TIER_LIST' ? (
                  ...
                  return (
                    <div key={tier} className="space-y-6">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-4 border-l-4 border-indigo-500 pl-4">
                        <div className={cn(
                          "w-16 h-10 sm:w-20 sm:h-12 rounded-2xl flex items-center justify-center text-xl sm:text-2xl font-black italic shrink-0",
                          info.color
                        )}>
                          {tier}
                        </div>
                      ...
                      <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 xl:grid-cols-9 gap-3 sm:gap-4">
                        {tierList[tier].map((entry) => (
                          ...
                          >
                            ...
                            <div className="relative z-10 w-12 h-12 sm:w-16 sm:h-16 rounded-3xl bg-[#111111]/80 border border-white/10 backdrop-blur-sm flex items-center justify-center overflow-hidden mb-2 shadow-xl group-hover:scale-110 transition-transform">
              ...
              >
                {recommendedTeams.map((team, idx) => (
                  <motion.div 
                    ...
                    className="group bg-[#111111]/50 border border-white/5 rounded-[40px] p-8 space-y-6 hover:bg-[#111111] transition-colors relative overflow-hidden"
                  >
                    ...
                    <div className="space-y-2 relative z-10">
                      ...
                      <p className="text-white/50 text-xs leading-relaxed line-clamp-2">{team.description}</p>
                    </div>
                    ...
                    <div className="grid grid-cols-4 gap-3 relative z-10">
                      {team.members.map((memberId) => (
                        <div key={memberId} className="flex flex-col items-center gap-2">
                           <div className={cn(
                             "w-full aspect-square rounded-3xl flex items-center justify-center overflow-hidden border-2 relative group/card shadow-md bg-[#111111]",
                      ...
                      <div className="flex items-center gap-4">
                        <div className="flex flex-col">
                           ...
                           <div className="flex gap-1 mt-1">
                              {[1, 2, 3, 4, 5].map(i => (
                                <div key={i} className={cn("w-1.5 h-1.5 rounded-full", i <= (idx === 4 ? 5 : idx === 3 ? 4 : 3) ? "bg-indigo-500" : "bg-[#1a1a1a]")} />
                              ))}
                           </div>
                        </div>
                      </div>
              ...
              >
                <div className="text-center max-w-2xl mx-auto space-y-4 mb-12">
                   ...
                   <p className="text-white/50 text-sm font-medium">Комбинируйте стихии для получения мощных эффектов. Правильное наложение статусов — ключ к победе в сложных сражениях.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {reactions.map((rxn, idx) => (
                    <motion.div 
                      ...
                      className="relative overflow-hidden bg-[#111111]/40 border border-white/5 rounded-3xl p-6 group hover:border-white/10 transition-all hover:bg-[#111111]/60"
                    >
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div className="p-3 bg-white/5 rounded-3xl border border-white/5 shadow-xl">
                          {rxn.icon}
                        </div>
                        <div className="flex gap-1">
                          {rxn.elements.map(el => (
                            <div key={el} className="p-1 bg-black/40 rounded-xl border border-white/10" title={el}>
                              {getElementIcon(el)}
                            </div>
                          ))}
                        </div>
                      ...
                      <p className="text-white/50 text-xs leading-relaxed font-medium">{rxn.desc}</p>
                ...
                <div className="bg-[#111111]/80 border border-white/10 p-8 rounded-[40px] border-dashed">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 text-sm text-white/50 font-medium">
