...
code = code.replace(/onClick=\{\(\) => setRoute\('STORY'\)\} className="/g, 'onClick={() => setRoute(\'STORY\')} className="tutorial-story ');
code = code.replace(/onClick=\{\(\) => setRoute\('MAP'\)\} className="/g, 'onClick={() => setRoute(\'MAP\')} className="tutorial-map ');
code = code.replace(/onClick=\{\(\) => setRoute\('ROSTER'\)\} className="/g, 'onClick={() => setRoute(\'ROSTER\')} className="tutorial-roster ');
code = code.replace(/onClick=\{\(\) => setRoute\('GACHA'\)\} className="/g, 'onClick={() => setRoute(\'GACHA\')} className="tutorial-gacha ');
code = code.replace(/onClick=\{\(\) => setActiveTab\('EVENTS'\)\} className={`/g, 'onClick={() => setActiveTab(\'EVENTS\')} className={`tutorial-events ');
code = code.replace(/onClick=\{\(\) => setRoute\('ABYSS'\)\} className="/g, 'onClick={() => setRoute(\'ABYSS\')} className="tutorial-abyss ');

code = code.replace(/import React, \{ useState \} from 'react';/, "import React, { useState, useEffect } from 'react';\nimport Joyride, { Step, CallBackProps, STATUS } from 'react-joyride';");

const joyrideSteps = `  const tutorialSteps: Step[] = [
    {
...
code = code.replace(/export default function HubMenu[\s\S]*?\{/, (match) => match + '\n' + joyrideSteps);
